...
## Angkor Runner - September 19, 2026

Added seventh original game with an endless jump/slide loop, three lives, gradual capped difficulty, distance and mango scoring, brief impact protection, persistent local bests, two large action controls, original sprite artwork and a 108 BPM soundtrack. Catalog/search/categories/recent/favorites, bilingual metadata and shared audio/cleanup are connected. Quick runner taps are retained for one frame without changing the existing games' controls.

Production build generates 28 metadata routes and splits the new engine. TypeScript, ESLint and secret scan passed. Unit checks include bounded jump/slide behavior, one-time pickups, swept collisions, game-over and restart, long-run object limits, and three minutes of automated obstacle avoidance with all three lives retained. Full-game images and mobile layout screenshots reviewed in outputs/review (ignored). Music analyser checks detect real browser audio with effects off; no subjective listening or physical-device testing performed.
