  ...
  const scene: Scene = {
    ...
    update(dt, input) {
      ...
      if (input.left || input.right) {
        ...
        input.pointer = null;
      } else if (input.pointer !== null) x = input.pointer;
      ...
      for (const item of items) {
        ...
        item.spin += dt;
        if (
          item.y >= config.catchY - 20 &&
          item.y <= config.catchY + 15 &&
          Math.abs(item.x - x) < 55
        ) {
    ...
    draw(ctx) {
      countryside(ctx);
      for (const item of items)
        fruit(ctx, item.x, item.y, item.type, Math.sin(item.spin) * 0.3);
      ...
      if (flash > 0) {
        ctx.fillStyle = 'rgba(188,55,55,.16)';
