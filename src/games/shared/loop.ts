...
export function createLoop(scene: Scene, options: GameOptions): Engine {
  const { canvas, onSnapshot, onFinish } = options;
  ...
  const pause = () => {
    if (scene.snapshot.phase === 'running') {
      scene.snapshot.phase = 'paused';
      clearInput();
      publish();
    }
  };
  ...
  const onKey = (e: KeyboardEvent) => {
    ...
    if (
      ...
      if (
        ...
        if (scene.snapshot.phase === 'running') pause();
        else if (scene.snapshot.phase === 'paused') {
      ...
      if (e.repeat) return;
      if (key === ' ' && e.type === 'keydown') setInput('action', true);
  ...
  function tick(now: number) {
    ...
    if (scene.snapshot.phase === 'over' && !finished) {
      finished = true;
      publish();
      onFinish({ ...scene.snapshot });
    }
  ...
  return {
    ...
    pause,
    resume: () => {
      if (scene.snapshot.phase === 'paused') start();
    },
    restart: () => {
      scene.reset();
      finished = false;
