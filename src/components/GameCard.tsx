...
import { Heart, ArrowUpRight, Keyboard, Mouse, Smartphone } from 'lucide-react';
...
export default function GameCard({ game }: { game: Game }) {
  ...
  return (
    <article className="game-card">
      <div className="cover">
        <Link to={'/games/' + game.id} aria-label={t(game.id + '.title')}>
          <picture>
            <source
              srcSet={
                '/art/' +
              ...
              sizes="(max-width: 650px) 90vw, 33vw"
            />
            <img
        ...
        <span className="category-tag">{t(game.category)}</span>
        <button
          className={
            'favorite-button ' + (favorites.includes(game.id) ? 'selected' : '')
          }
          aria-label={t('favorite') + ' ' + t(game.id + '.title')}
      ...
      <div className="card-body">
        <div className="row between">
          <h3>
            <Link to={'/games/' + game.id}>{t(game.id + '.title')}</Link>
          </h3>
              ...
              const Icon =
                i === 'keyboard'
                  ...
                  : i === 'mouse'
                    ...
                    : Smartphone;
              return (
                <span key={i} role="img" title={t(i)} aria-label={t(i)}>
                  <Icon size={16} />
                </span>
              );
            })}
